...
describe('public.users authorization', () => {
  ...
  it('без учётки не читает чужой ряд', async () => {
  ...
  it('аутентифицированный пользователь не читает чужой ряд', async () => {
  ...
  it('аутентифицированный пользователь не меняет чужой ряд', async () => {
  ...
  it('отвергает role, права и неизвестные поля на самообслуживании', async () => {
  ...
  it('не даёт пользователю менять баланс, почту и the_p через свой маршрут', async () => {
    const id = await seed('james@example.com', 'demo')
