...
Пройдено 25 этапов. Работают: создание и восстановление кошелька,
аккаунты, сети и выбор RPC-узла, балансы, токены, история переводов,
отправка нативной валюты, портфель с оценкой, подключение приложений
по WalletConnect, имена ENS, резервное копирование секретов,
модуль безопасности.

Хранилище **постоянное**: кошелёк переживает перезагрузку вкладки
и открывается тем же паролем.

**Пользоваться с настоящими средствами пока рано.** Не реализованы
отслеживание статуса отправленной транзакции, отправка токенов ERC-20
и NFT, ускорение и отмена застрявшей транзакции; нет ограничения числа
попыток входа и списка проверенных контрактов.
...
| Проверка | Значение |
| --- | --- |
| Модульных и интеграционных тестов | 1451 |
| Сквозных тестов (Chromium) | 17 |
