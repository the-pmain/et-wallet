...
**Курсы опрашиваются раз в минуту тем же выключателем.** «Реального
времени» у источника курсов нет: ни бесплатный доступ, ни
демонстрационный не отдают потока — ни веб-сокета, ни длинного
соединения. Остаётся опрос, и минута — не круглое число, а граница
бесплатного доступа: курс запрашивается отдельно для нативной валюты
и **по одному запросу на каждый токен** (пакет больше единицы
отвергается отказом `10012` — измерено). Кошелёк с тремя токенами
тратит четыре обращения на такт, то есть 240 в час открытого экрана
при месячном пределе демонстрационного ключа в 10 000. Шаг
в пятнадцать секунд сжёг бы этот предел за десять часов.

Отказ разводит такты вдвое, до восьми минут: `429` на превышение
частоты не лечится повторением с прежним шагом. Первый успех
возвращает обычную минуту.

Фоновый такт **не сбрасывает кэш курсов**, в отличие от кнопки
обновления. Разница существенная: сброшенный кэш при отказе источника
оставляет экран без оценки, и один неудачный запрос стирал бы верное
значение без всякого действия владельца. По нажатию это честно —
человек сам попросил; само по себе — нет.

Рядом с оценкой показано **время котировки**. Без него живое число
неотличимо от замершего: при отказе источника на экране остаётся
прежний курс, и выглядит он точно так же.

Согласие на курсы **называет и опрос**: в перечне того, что узнаёт
сервис, отдельной строкой стоит длительность сессии. Разовое обращение
и обращение раз в минуту — разные вещи, и собрать согласие на первое,
делая второе, значило бы обмануть.
