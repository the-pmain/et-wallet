...
describe('toDappRequest: подпись сообщения', () => {
  ...
  it('eth_sign отвергается независимо от порядка параметров', () => {
    /* Метод подписывает произвольные 32 байта без префикса: приложение
       может прислать хэш транзакции и получить подпись под ней, ничего
       не показав владельцу. Отвергается на входе, а не разбирается
       «как получится». */
    expect(toDappRequest(raw('eth_sign', [OWNER, 'Войти']))).toBeNull()
    expect(toDappRequest(raw('eth_sign', ['Войти', OWNER]))).toBeNull()
  })

  it('eth_sign отвергается и с шестнадцатеричным значением', () => {
    /* Именно так выглядит опасный случай: тридцать два байта, похожие
       на хэш транзакции. */
