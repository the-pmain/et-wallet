...
export function mapProviderError(error: unknown, chainId: ChainId): Error {
  ...
  if (isEthersError(error)) {
    ...
    /* Узел ответил кодом HTTP, но не ответом JSON-RPC: тела с `code`
       и `message` в нём нет — иначе оно было бы разобрано выше.
       Это отказ узла, а не отрицательный ответ по существу запроса.

       ПОЧЕМУ РАЗЛИЧИЕ ВАЖНО. `FailoverProvider` перебирает резервные
       адреса только при `ProviderUnavailableError`, и обоснованно:
       нехватка средств и откат вызова не зависят от того, кого
       спрашивать, — второй узел ответит то же самое. Но «500» ничего
       не говорит о состоянии цепи, он говорит о состоянии узла, и
       соседний узел вполне может ответить. Возвращая здесь `RpcError`,
       мы выдавали отказ узла за его ответ и тем самым запрещали
       переключение: резервные адреса были настроены и не использовались.

       Проявлялось это на выборке журналов — `eth_getLogs` тяжелее
       прочих вызовов и отваливается первым, — но задето было всё.

       Код `-32603` в прежней ветке был выдуман нами: узел его не
       присылал. Подменять отсутствующий ответ придуманным кодом —
       та же ошибка, что подменять неизвестное нулём.

       Исключение адреса при этом не навсегда: `RpcManager` ставит его
       на выдержку, а когда на выдержке оказались все, берёт полный
       список заново. */
    if (isError(error, 'SERVER_ERROR')) {
      /* Текст берём у библиотеки: он называет код состояния HTTP
         («server response 500») и потому говорит о случившемся больше,
         чем общая фраза об отсутствии адресов, — а адреса при этом
         могут быть целы, отказал лишь один узел на одном запросе. */
      return new ProviderUnavailableError(chainId, {
        cause: error,
        /* Обрезка не косметика: библиотека склеивает код состояния
           с пояснением, и при пустом пояснении остаётся хвостовой
           пробел. Он попадает в кавычки на экране истории. */
