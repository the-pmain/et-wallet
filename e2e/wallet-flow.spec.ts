...
test.describe('Сквозной путь: создание и работа кошелька', () => {
  ...
  test('перезагрузка страницы теряет кошелёк: хранилище в памяти', async ({ page }) => {
    /* Известное ограничение текущего этапа, а не дефект. Проверка
       закрепляет его: молчаливое появление постоянного хранилища
       изменит поведение, о котором предупреждает интерфейс. */
    await importWallet(page)
