...
export interface IWalletSession {
  ...
  /**
   * Перезапрашивает курсы, минуя кэш. Вызывается по нажатию.
   *
   * СБРОС КЭША ЗДЕСЬ УМЕСТЕН, А В ФОНОВОМ ОПРОСЕ — НЕТ. Стёртый кэш
   * означает, что при отказе источника показывать становится нечего:
   * прежние котировки уже нет, новые не пришли, и оценка на экране
   * сменяется словами «получить не удалось». Для нажатия это честно —
   * человек сам попросил и видит последствие. Фоновый опрос кэш
   * не трогает: один неудачный запрос не должен стирать верное
   * значение без всякого действия владельца.
   */
