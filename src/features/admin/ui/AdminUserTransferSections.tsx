...
function UserReceivingsSection({
  ...
  const [asset, setAsset] = useState(defaultTransferAsset)
  const [amount, setAmount] = useState('')
  const [walletCodename, setWalletCodename] = useState(
    walletOptions[0]?.value ?? MOCK_WALLET_CODENAME,
  )
  ...
  async function createReceiving(): Promise<void> {
    ...
    const selected =
      walletOptions.find((item) => item.value === walletCodename) ??
      walletOptions[0] ??
      mockWalletChoice()
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Deposit any amount of any ticker. Pick a wallet or use the default mock wallet. The record
          appears on the owner&apos;s Activity page.
        </p>
      </CardHeader>
      <CardContent className="flex flex-col gap-4">
        <div className="grid gap-3 sm:grid-cols-2">
