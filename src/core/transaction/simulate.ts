 ...
 *
 * ПОЧЕМУ НЕ НУЖЕН СТОРОННИЙ СЕРВИС. `eth_simulateV1` — обычный метод
 * узла, с которым кошелёк уже разговаривает. Обращение к службе вроде
 * Tenderly сообщило бы ещё одному оператору адрес владельца и его
 * намерение, ничего не добавив по существу.
 *
