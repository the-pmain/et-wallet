 ...
 *
 * СРОК ГОДНОСТИ ВДВОЕ КОРОЧЕ ПРОМЕЖУТКА МЕЖДУ ОПРОСАМИ, И ЭТО СВЯЗЬ,
 * А НЕ СОВПАДЕНИЕ. Кошелёк опрашивает курсы раз в минуту, пока экран
 * открыт. Будь срок годности тоже минутой, опрос заставал бы котировку
 * ровно на границе: при малейшем опережении она считалась бы свежей,
 * запрос не уходил бы вовсе, и обновление шло бы через раз — то есть
 * раз в две минуты вместо обещанной одной.
 *
 * Тридцать секунд убирают эту границу: к моменту опроса котировка
 * заведомо просрочена, и запрос уходит каждый раз. Лишних обращений
 * это не добавляет — их частоту задаёт опрос, а не срок годности.
 * Переходы между экранами внутри полуминуты по-прежнему обходятся
 * кэшем.
 */
...
export class PriceService implements IPriceService {
  ...
  constructor(dependencies: IPriceServiceDependencies) {
    ...
    this.#ttlMs = dependencies.ttlMs ?? PRICE_QUOTE_TTL_MS
  }
